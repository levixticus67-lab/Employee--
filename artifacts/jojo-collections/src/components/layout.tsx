...
import { ShoppingBag, Menu, X, User, LogOut, Heart, MessageCircle } from "lucide-react";
      ...
      <header className="sticky top-0 z-50 glass-panel-heavy border-b border-white/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center h-16 gap-6">
            ...
            {/* Logo */}
            <Link href="/" className="flex-shrink-0 text-xl font-serif text-blue-950 font-bold tracking-widest">
              JOJO
            </Link>
              ...
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`text-xs font-medium uppercase tracking-wider transition-colors ${
                    location === link.href ? "text-blue-900 font-semibold" : "text-blue-800/60 hover:text-blue-950"
                  }`}
                >
                  {link.label}
                </Link>
            ...
            <div className="hidden md:flex items-center gap-2 ml-auto">
              ...
              <select
                ...
                className="text-xs font-medium text-blue-800/60 bg-transparent border-0 focus:outline-none cursor-pointer hover:text-blue-950 pr-1"
              >
              ...
              {user ? (
                <div className="flex items-center gap-1 border-l border-blue-900/10 pl-2">
                  <span className="flex items-center gap-1.5 text-xs text-blue-900 font-medium px-2">
                  ...
                  <button
                    ...
                    title="Sign out"
                    className="p-1.5 text-blue-800/50 hover:text-red-600 transition-colors rounded-lg"
                  >
                ...
                <div className="flex items-center gap-2 border-l border-blue-900/10 pl-2">
                  <Link href="/login" className="text-xs font-medium text-blue-800/60 hover:text-blue-950 px-2 py-1.5">
                    Sign In
                  </Link>
                  <Link href="/signup" className="text-xs font-medium px-3 py-1.5 rounded-full bg-blue-600 text-white hover:bg-blue-700 transition-colors">
                    Sign Up
                  </Link>
                </div>
              )}
            ...
            <div className="md:hidden flex items-center gap-1 ml-auto">
              <Link href="/wishlist" className="relative p-2 text-blue-900">
                ...
                {wishlistCount > 0 && (
                  <span className="absolute top-0.5 right-0.5 flex items-center justify-center w-3.5 h-3.5 text-[9px] font-bold text-white bg-red-500 rounded-full">{wishlistCount}</span>
                )}
              </Link>
              <Link href="/cart" className="relative p-2 text-blue-900">
                <ShoppingBag className="w-5 h-5" />
                {totalItems > 0 && (
                  <span className="absolute top-0.5 right-0.5 flex items-center justify-center w-3.5 h-3.5 text-[9px] font-bold text-white bg-blue-600 rounded-full">{totalItems}</span>
                )}
              </Link>
            ...
            <div className="px-4 pt-2 pb-4 space-y-1">
              {navLinks.map((link) => (
                <Link key={link.href} href={link.href} onClick={() => setIsMobileMenuOpen(false)}
                  className="block px-3 py-2 text-sm font-medium text-blue-900 hover:bg-white/20 rounded-md uppercase tracking-wider">
                  {link.label}
                </Link>
              ))}
              ...
              {user ? (
                <>
                  ...
                  <button type="button" onClick={() => { setIsMobileMenuOpen(false); handleLogout(); }}
                    className="flex items-center gap-2 w-full text-left px-3 py-2 text-sm font-medium text-red-600 hover:bg-white/20 rounded-md">
