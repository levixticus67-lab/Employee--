import { useState, useEffect } from "react";
  ...
  import { Plus, Edit2, Trash2, Upload, X, Search, Filter } from "lucide-react";
  ...
  export default function AdminProducts() {
    ...
    const clearFilters = () => { setSearch(""); setFilterCategory("All"); setFilterCollection("All"); };
    const hasActiveFilter = search || filterCategory !== "All" || filterCollection !== "All";
    ...
    return (
      <AdminLayout>
        ...
        {/* ── Filter / Search Bar ───────────────────────────────────────────── */}
        <div className="glass-panel-heavy rounded-2xl p-4 mb-6 border-white/50">
          <div className="flex flex-col md:flex-row gap-3 items-start md:items-center justify-between">
            {/* Category pills */}
            <div className="flex flex-wrap gap-2">
              {["All", ...CATEGORIES].map((cat) => (
                <button key={cat} onClick={() => setFilterCategory(cat)}
                  className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all ${filterCategory === cat ? "bg-blue-600 text-white shadow-md shadow-blue-600/30" : "glass-card text-blue-900 hover:bg-white/40"}`}>
                  {cat}
                </button>
              ))}
            </div>
            ...
            <div className="flex items-center gap-2 w-full md:w-auto">
              ...
              <div className="relative flex-1 md:w-56">
                <input
                  ...
                  className="w-full glass-card rounded-full py-2 pl-9 pr-4 text-blue-950 placeholder:text-blue-800/40 focus:outline-none focus:ring-2 focus:ring-blue-400/50 border-white/40 bg-white/30 text-sm"
                />
                <Search className="absolute left-3 top-2.5 w-4 h-4 text-blue-700/50" />
              </div>

              {/* Advanced filter toggle */}
              <button onClick={() => setShowFilters(!showFilters)}
                className={`flex items-center gap-1.5 px-3 py-2 rounded-full text-sm font-medium transition-all ${showFilters ? "bg-blue-600 text-white" : "glass-card text-blue-900 hover:bg-white/40"}`}>
                <Filter className="w-4 h-4" /> Filters
              </button>

              {/* Clear */}
              {hasActiveFilter && (
                <button onClick={clearFilters} className="text-sm text-blue-500 hover:text-blue-700 font-medium whitespace-nowrap">
                  Clear
                </button>
              )}
            </div>
          ...
          {/* Collection dropdown (advanced) */}
          {showFilters && collections.length > 0 && (
            <div className="mt-3 pt-3 border-t border-white/20 flex items-center gap-3">
              <label className="text-xs font-medium text-blue-900/60 uppercase tracking-wider whitespace-nowrap">Collection</label>
              <select
                value={filterCollection}
                onChange={(e) => setFilterCollection(e.target.value)}
                className="glass-card rounded-lg px-3 py-1.5 text-sm text-blue-950 border-white/40 focus:ring-2 focus:ring-blue-400 focus:outline-none bg-white/30">
        ...
        {!isLoading && (
          <p className="text-xs text-blue-800/50 mb-3">
            {filtered.length} of {products?.length ?? 0} product{filtered.length !== 1 ? "s" : ""}
          </p>
        )}
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-white/20">
                {isLoading ? (
                  ...
                  <tr><td colSpan={6} className="px-6 py-8 text-center text-blue-800">No products match your filters.</td></tr>
                ) : (
                ...
                <>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <div>
                      ...
                      <input type="number" min="0" value={form.stock} onChange={(e) => setForm({ ...form, stock: Number(e.target.value) })} className="w-full glass-card rounded-lg px-3 py-2 text-blue-950 border-white/40 focus:ring-2 focus:ring-blue-400 focus:outline-none" />
                    </div>
                  </div>
                  <div>
                  ...
                  <div className="flex items-center gap-3">
                    <input type="checkbox" id="featured" checked={form.featured} onChange={(e) => setForm({ ...form, featured: e.target.checked })} className="w-4 h-4 rounded accent-blue-600" />
                    <label htmlFor="featured" className="text-sm font-medium text-blue-900/80">Featured product</label>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-blue-900/80 mb-2">Product Image</label>
                    <div className="flex items-center gap-3">
                      {form.imageUrl && (
