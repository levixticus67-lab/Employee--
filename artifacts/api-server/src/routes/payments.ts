...
router.post("/payments/initiate", async (req, res) => {
          ...
          if (c.active && !expired && (c.maxUses === null || c.uses < c.maxUses) && subtotal >= c.minOrder) {
            ...
            if (!isOnline) {
              ...
              tx.set(firestore.collection(COLLECTIONS.couponUsages).doc(), {
                couponId: cDoc.id,
                userId: canonicalUid,  // Firebase UID preferred
  ...
  try {
    ...
    const submitRes = await fetch(`${PESAPAL_BASE}/api/Transactions/SubmitOrderRequest`, {
      ...
      body: JSON.stringify({
        id: txRef, currency: "UGX", amount: Math.round(finalTotal),
    ...
    await firestore.collection(COLLECTIONS.orders).doc(orderId).update({ pesapalTrackingId: submitData.order_tracking_id });
  ...
  async (req, res) => {
    ...
    try {
      ...
      await firestore.runTransaction(async (tx) => {
        ...
        tx.update(orderRef, {
          paymentStatus: "paid", amountPaid: fresh.total, pesapalTrackingId: trackId,
          status: "processing",
