...
router.post("/admin/auth/login", (req, res) => {
  ...
  const emailOk    = (email ?? "").toLowerCase().trim() === expectedEmail.toLowerCase().trim();
  const passwordOk = (password ?? "") === expectedPassword;
  if (!emailOk || !passwordOk) { res.status(401).json({ error: "Invalid admin email or password" }); return; }
