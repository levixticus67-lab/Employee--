import { Router, type IRouter } from "express";
  ...
  router.post("/auth/signup", async (req, res) => {
    ...
    const existing = await firestore.collection(COLLECTIONS.users).where("email", "==", normalizedEmail).limit(1).get();
    if (!existing.empty) { res.status(400).json({ error: "An account with that email already exists" }); return; }

    const passwordHash = await bcrypt.hash(password, 10);
    const data: UserDoc = {
      name, email: normalizedEmail, passwordHash, createdAt: Timestamp.now(),
      emailVerified: firebaseUid ? false : true,
      firebaseUid: firebaseUid ?? null, phoneNumber: null,
    };
    const ref = await firestore.collection(COLLECTIONS.users).add(data);
    if (!firebaseUid) req.session.userId = ref.id;
  ...
  router.post("/auth/login", async (req, res) => {
    ...
    const normalizedEmail = email.toLowerCase().trim();
    const snap = await firestore.collection(COLLECTIONS.users).where("email", "==", normalizedEmail).limit(1).get();
    ...
    if (user.emailVerified === false) {
      res.status(403).json({ error: "Please check your inbox and verify your email link to activate your account and log in." }); return;
    }
  ...
  router.post("/auth/login-firebase", async (req, res) => {
    ...
    catch (err) { req.log.warn({ err }, "Firebase token verification failed"); res.status(401).json({ error: "Invalid or expired authentication token" }); return; }
    ...
    const userDoc = snap.docs[0]!;
    const user = userDoc.data() as UserDoc;
    await firestore.collection(COLLECTIONS.users).doc(userDoc.id).update({ emailVerified: true, firebaseUid: decodedToken.uid });
  ...
  // ── Admin Auth ────────────────────────────────────────────────────────────────
  // Verifies a Firebase ID token and checks the email matches ADMIN_EMAIL env var.
  // This ensures only the designated admin Firebase account can open an admin session.
  router.post("/admin/auth/login", async (req, res) => {
    const { firebaseIdToken } = req.body as { firebaseIdToken?: string };
    if (!firebaseIdToken) {
      res.status(400).json({ error: "firebaseIdToken is required" }); return;
    }
